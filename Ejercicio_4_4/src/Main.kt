...
fun main() {
    ...
    // Muestra coches por pantalla
    println(coche1)
    println(coche2)

    // Intenta modificar los valores con condiciones incorrectas
    try {
        coche1.color = ("Verde")
    ...
    try {
        coche1.matricula = ("AB12345")
    } catch (e: IllegalArgumentException) {
        println("Error: ${e.message}")
    }

    try {
        val coche3 = Coche("", "Focus", "Blanco", 120, 4, "ABC1234")
    } catch (e: IllegalArgumentException) {
        println("Error: ${e.message}")
    }
